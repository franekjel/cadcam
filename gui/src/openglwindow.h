...
class OpenGLWindow : public QOpenGLWindow, public QOpenGLFunctions_4_2_Core
{
    Q_OBJECT
public:
    explicit OpenGLWindow(const std::vector<std::pair<QVector3D, QVector3D>>& _points, const std::vector<std::pair<QMatrix4x4, QMatrix4x4>>& _matrices, std::string model = "", QWindow* parent = 0);
    ~OpenGLWindow();
...
private:
    ...
    std::shared_ptr<Object> ReadModel(std::string s);
